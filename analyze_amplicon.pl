...
##v2
unless (getopts("s:k:i:o:l:b:c:r:a:w:v:t:m:d:x:j:y:z:u:n:h", \%opts))
{
        printhelp();
...
########################################
#base commands, default:Cornell BioHPC path
########################################
our $trimmomatic_cmd = "java -jar /programs/Trimmomatic-0.36/trimmomatic-0.36.jar";
our $trimmomatic_adapter = "/programs/Trimmomatic-0.36/adapters/amplicon-SE.fa";
our $flash_cmd = "/programs/FLASH2/flash2";
...
our $bcf_cmd = "bcftools"; ### part of the samtools package
our $vcfutils_cmd = "vcfutils.pl"; ### part of the samtools package
our $clustalw_cmd = "clustalw";
our $clustalo_cmd = "/programs/clustalo";
our $vcfconcatcmd = "vcf-concat"; ### part of vcftools
our $picard_path = "/programs/bin/picard-tools/";
...
if ($opts{"n"}) 
{
	...
	if (-e $t) 
	{
		$trimmomatic_adapter =$t;
	}
	else
	{
		print "Error: Adapter file $t does not exist!\n";
		exit;
	}
...
if ($opts{"v"}) 
{
	...
	if ($t=~/^(\d+):(\d+):([0-9.]+)$/)
	{
		...
		$flash_cmd = "$flash_cmd -t $threads $allow_outies -m $min_overlap_length -M $max_overlap_length -x $contiging_mismatch ";
	}
	else
	{
		printhelp();
...
if ($opts{"y"}) 
{
	...
	unless ($mismatches=~/^\d$/) 
	{
	   	print "Mismatches (-y) must be a single digit number.\n";
		exit;
	}
}
...
if ($opts{"z"}) 
{
	...
	{
		print "Remove_primer setting (-z) must be 'y' or 'n'.\n";
		exit;
	}
}
...
if (defined $opts{"m"}) 
{
	...
	elsif ($t[0]=~/clustalo/i) 
	{
		...
		$runmsa = "$clustalo_cmd --threads=$threads -i xxxxx --seqtype=DNA --output-order=input-order --outfmt=$t[1] -o xxxxx.alignment $my_wrap --force>& /dev/null";
	}
	else
	{
		printhelp();
...
while (<IN>)
{
	...
	if ((defined $key) && (defined $seq) && ($key=~/\w/) && ($seq=~/\w/)) 
	{
		...
		if ($key=~/(\S+)_([FR])$/) 
		{
			...
			$primerLength{$gene}{$strand} = length($seq);
			if ($strand eq "F") 
			{
				push @genes, $gene;
			}
...
########switch between concatenate or assemble the paired end, if concatenate, truncate read to defined length
	...
	LOOPi2:foreach my $sample (@samples) 
	{
		...
		if ($contigmode=~/^[13]$/) 
		{
			...
			{
				my $filter_by_length_cmd = "awk 'BEGIN {OFS = \"\\n\"} {header = \$0 ; getline seq ; getline qheader ; getline qseq ; if (length(seq) >= $minimum_length) {print header, seq, qheader, qseq}}' < $assembledir/$sample.extendedFrags.fastq > t.fastq";
				system ($filter_by_length_cmd);

				#print ("cp t.fastq $assembledir/$sample.extendedFrags1.fastq");
				system ("cp t.fastq $assembledir/$sample.extendedFrags.fastq");
...
if (((! $opts{"i"})) || (($opts{"i"}) && (! ($opts{"i"}=~/3/))))
{
	...
	if ($opts{"b"}) 
	{
		...
		$bF_length =   $barcode_length;
		$bR_length =   $barcode_length;
	}

	###make forward barcode file
	## get mininum length
	my $min_len = 99;
	my $max_len = 0;
	foreach my $gene (@genes) 
	...
	if (defined $barcode_length) 
	{
		...
		{
			print STAT "Use $bF_length nt to split genes by forward primer in sequence file\n";
			print "  Use $bF_length bp to split locus by forward primer in  sequence file\n"; 
		}
	}
	else
	{
	...
	foreach my $gene (@genes) 
	{
		my $barcode = substr ($gene2seq{$gene}{"F"}, 0, $bF_length);
	...
	if ($reverse_flag==1) 
	{
		...
		foreach my $gene (@genes) 
		{
			my $barcode = $gene2seq{$gene}{"R"};
			unless ($barcode)
			{
				print "Error: gene $gene does not have reverse barcode\n";
				exit;
			}
			my $blen = length $barcode;
			if ($min_len2 > $blen) 
		...
		print "  Reverse primer length:  $min_len2 to $max_len2\n";
		if (defined $barcode_length) 
		{
			if ($barcode_length > $min_len2) 
			{
				print STAT "The provided PCR primer barcode length $barcode_length longer than the shortest primer R length $min_len2. The size $min_len2 is used.\n";
				print  "  The provided PCR primer barcode length $barcode_length longer than the shortest primer R length $min_len2. The size $min_len2 is used.\n";
				$bR_length = $min_len2;
			}
			else
			{
				print STAT "Use $bR_length nt to split genes in  sequence file\n";
				print "  Use $bR_length bp to split locus in  sequence file\n";
			}
		}
		else
		{
			$bR_length = $min_len2;
			print STAT "Use smallest primer length  $min_len2 bp to split locus in  sequence file\n";
			print "  Use shortest primer length  $min_len2 bp to split locus in  sequence file\n";
		}



		
		my %check_reduncancy2 = ();
		foreach my $gene (@genes) 
		{
			open (B2, ">$splitdir/${gene}_barcode2.txt") || die "Error: cannot make barcode file. Writing to $splitdir/barcode2.txt failed\n";
			my $barcode = revcom2(substr ($gene2seq{$gene}{"R"}, 0, $bR_length));
			if (exists $check_reduncancy2{$barcode}) 
			{
				print "Error: $barcode is duplicated, cannot uniquely identify a gene!\n";
				exit;
			}
			$check_reduncancy2{$barcode} = "";
			print B2 "RRR\t$barcode\n"; 
			close B2;
		}
	...
	LOOPi3:foreach my $sample (@samples) 
	{
		...
		my $split_out = "";
		my $split_out2 = "";
		my $unmatched_R_primer = 0;
		if ((-e "$assembledir/$sample.extendedFrags.fastq") && ((-s "$assembledir/$sample.extendedFrags.fastq")>0))
		{
			$command_contig = "$split_cmd --bcfile $splitdir/barcode.txt --prefix $splitdir/${sample}_tig_ --bol --mismatches $mismatches < $assembledir/$sample.extendedFrags.fastq";
			$split_out .= `$command_contig  2>&1`;
			 $unmatched_R_primer = 0;
			...
			foreach my $gene (@genes) 
			{
				if (-e "$splitdir/${sample}_tig_$gene") 
				{
					if ($reverse_flag==1) 
					{
						$command_contig = "$split_cmd --bcfile $splitdir/${gene}_barcode2.txt --prefix $splitdir/ --eol --mismatches $mismatches < $splitdir/${sample}_tig_$gene";
						$split_out2 = `$command_contig  2>&1`;
						if( $split_out2 =~/unmatched\s+(\d+)/)
						{
							$unmatched_R_primer += $1;
						}
						if( $split_out2 =~/RRR\s+(\d+)/)
						{
							$split_out.= "$gene\t$1\tloc\n";
						}

						if (-e "$splitdir/RRR") 
						{
							#print $command_contig, "\n"; exit;
							system ("mv $splitdir/RRR $splitdir/${sample}_tig_$gene");

						}
						else
						{
							 system ("touch $splitdir/${sample}_tig_$gene");	
						}
						
					}
					if ($remove_primer==1) 
					{
						trim_primer("$splitdir/${sample}_tig_$gene", $primerLength{$gene}{"F"}, $primerLength{$gene}{"R"});	
					}
				}
			}
		}
		...
		{
			$unmatched_R_primer = 0;
			$command_concat = "$split_cmd --bcfile $splitdir/barcode.txt --prefix $splitdir/${sample}_cat_ --bol --mismatches $mismatches < $assembledir/$sample.concat.fastq";
			...
			foreach my $gene (@genes) 
			{
				if (-e "$splitdir/${sample}_cat_$gene") 
				{
					if ($reverse_flag==1) 
					{
						$command_contig = "$split_cmd --bcfile $splitdir/${gene}_barcode2.txt --prefix $splitdir/ --eol --mismatches $mismatches < $splitdir/${sample}_cat_$gene";
						$split_out2 = `$command_contig  2>&1`;
						if( $split_out2 =~/unmatched\s+(\d+)/)
						{
							$unmatched_R_primer += $1;
						...
						if (-e "$splitdir/RRR") 
						{
							system ("mv $splitdir/RRR $splitdir/${sample}_cat_$gene");	
						}
						else
						{
							 system ("touch $splitdir/${sample}_cat_$gene");	
						}
											
					}
					if ($remove_primer==1) 
					{
						trim_primer("$splitdir/${sample}_cat_$gene", $primerLength{$gene}{"F"}, $primerLength{$gene}{"R"});	
					}
				}
			}
		...
		if ($split_out =~/Count/)
		{
			...
			print "$sample\tTotal: $total_count\tUnmatched_F: $unmatched_count\tUnmatched_R: $unmatched_R_primer\n";
			print STAT "$sample\tTotal: $total_count\tUnmatched_F: $unmatched_count\tUnmatched_R: $unmatched_R_primer\n";
			print LOG "$split_out\n";
...
if (((! $opts{"i"})) || (($opts{"i"}) && (! ($opts{"i"}=~/7/))))
{
	...
	open OUT, ">$outdir/hap_genotype_matrix";
	print OUT "\t";
...
if (((! $opts{"i"})) || (($opts{"i"}) && (! ($opts{"i"}=~/8/))))
{
	...
	LOOPi8:foreach my $gene (@genes)
	{
		...
		LOOPi82:foreach my $sample (@samples) 
		{
			...
			{
				system ("java -jar ${picard_path}MergeSamFiles.jar INPUT=$samplegene2fastadir/${gene}.sam INPUT=$samplegene2fastadir/mygenesample.sam SORT_ORDER=unsorted OUTPUT=$samplegene2fastadir/t.sam QUIET=true VERBOSITY=ERROR");
		...
		system ("java -jar ${picard_path}picard.jar SamFormatConverter INPUT=$samplegene2fastadir/${gene}.sam OUTPUT=$samplegene2fastadir/${gene}.bam QUIET=true VERBOSITY=ERROR ");
		system ("java -jar ${picard_path}picard.jar SortSam INPUT=$samplegene2fastadir/${gene}.bam OUTPUT=$samplegene2fastadir/${gene}.sorted.bam SORT_ORDER=coordinate QUIET=true VERBOSITY=ERROR");
		system ("java -jar ${picard_path}picard.jar CleanSam INPUT=$samplegene2fastadir/${gene}.sorted.bam OUTPUT=$samplegene2fastadir/${gene}.clean.bam QUIET=true VERBOSITY=ERROR");
		system ("java -jar ${picard_path}picard.jar BuildBamIndex INPUT=$samplegene2fastadir/${gene}.clean.bam QUIET=true VERBOSITY=ERROR");
		system ("$samtools_cmd faidx $samplegene2fastadir/ref_${gene}.fasta");
		system ("java -jar ${picard_path}picard.jar CreateSequenceDictionary REFERENCE=$samplegene2fastadir/ref_${gene}.fasta OUTPUT=$samplegene2fastadir/ref_${gene}.dict QUIET=true VERBOSITY=ERROR");
		system ("java -jar ${gatk_path}GenomeAnalysisTK.jar -R $samplegene2fastadir/ref_${gene}.fasta -T UnifiedGenotyper -o $vcfdir/${gene}.vcf -I $samplegene2fastadir/${gene}.clean.bam");
	}
...
sub printhelp
{
	...
	print "-y: mismatches in primer matching (number, default 0)\n";
	print "-z: remove both primers from haplotypes (y or n, default n)\n";
	...
	print "-n: fasta file of adapter sequence, default is the i7 i5 adapter file /programs/Trimmomatic-0.36/adapters/amplicon-SE.fa \n";
	...
	print "-u: parameter for contiging, allow outie\n";
...
sub trim_primer
{
	...
	open TMP, $inputfile;
	open TMPOUT, ">$tempoutfile";
	LOOPTMP:while (my $line1 =<TMP>) 
	{
		my $line2 =<TMP>;
		my $line3 =<TMP>;
		my $line4 =<TMP>;
		...
		$line2=~s/\s+$//;
		$line4=~s/\s+$//;
		my $remainlen = length($line2) - $left - $right;
		if ($remainlen <= 0) 
		{
			next LOOPTMP;
		}
		$line2 = substr($line2, $left, $remainlen);
		$line4 = substr($line4, $left, $remainlen);

		print TMPOUT "$line1$line2\n$line3$line4\n" 
		
	}
	close TMP;
	close TMPOUT;
	system ("mv $tempoutfile $inputfile");
